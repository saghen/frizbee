...
#[inline(always)]
pub fn match_haystack_unordered(needle: &[(u8, u8)], haystack: &[u8]) -> (bool, usize) {
    let len = haystack.len();
    match len {
        0 => return (true, 0),
        1..=7 => return (match_haystack(needle, haystack), 0),
        _ => {}
    }
    ...
    for start in (0..len).step_by(16) {
        let (chunk_start, chunk_end) = chunk_range(start, len);
    ...
    (needle_idx >= needle.len(), skipped_chunks)
}

/// Unordered prefilter with typo support, matching SIMD prefilter behavior.
#[inline(always)]
pub fn match_haystack_unordered_typos(
    needle: &[(u8, u8)],
    haystack: &[u8],
    max_typos: u16,
) -> (bool, usize) {
    ...
    match len {
        0 => return (true, 0),
        1..=7 => return (match_haystack_typos(needle, haystack, max_typos), 0),
        _ => {}
    }
