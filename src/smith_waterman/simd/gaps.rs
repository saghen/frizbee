...
pub unsafe fn propagate_horizontal_gaps<Simd256: Vector256>(
    ...
    // shift by 4 elements (8 bytes), decay by 4
    let shifted = row.shift_right_padded_u16::<8>(adjacent_row);
    let gap_penalty = Simd256::blendv(
        Simd256::splat_u16(gap_extend_penalty * 8),
        Simd256::splat_u16(gap_open_penalty + gap_extend_penalty * 7),
        match_mask.shift_right_u16::<8>(),
    );
    let decayed = shifted.subs_u16(gap_penalty);
    row.max_u16(decayed)
}
