...
matches = matcher.match(["fooBar", "foo_bar", "barfoo", "prelude"])
...
patterns = frizbee.parse_query("foo !^bar")
for pattern in patterns:
    pattern.max_typos = len(pattern.needle) // 4
matcher = frizbee.Matcher(patterns)
```

## Performance

These bindings achieve native performance to the Rust crate, after a one-time copy of strings to Rust. On the Chromium file list (1.4M haystacks, needle "linux", limit 1000):

```
new Haystacks(string[]):               33.99 ms
match (string[]):                      58.47 ms/iter
match (Haystacks):                     23.60 ms/iter
match(threads=8) (string[]):           40.14 ms/iter
match(threads=8) (Haystacks):           4.52 ms/iter
```
